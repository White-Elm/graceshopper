...
const AdminCustomers= ({customers}) =>{
    ...
    return(
        <div>
            ...
            <div className='AdminManage-Cust-Title'>Manage Customers</div>
            <ul className='adminCust-Ul'>
            {customers.map(customer =>{
                return(
                    <li className='adminCust-Li'key={customer.id}>
                        <div className='adminCustCard'>
                        <img className='adminCust-img' src={Math.round(Math.random()*(custImgs.length)-1)>0 ? custImgs[Math.round(Math.random()*(custImgs.length)-1)] : 'https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQQlhpPodekhaRM5kn4gbZ2HyA-NyXIqZKOaqIEvn1Xg-eJQjk9hV6XNzvdJgwfw4X4gVA&usqp=CAU'}/>
                        <br/>
