...
const productArr = [];
for(let i = 0; i<5; i++){
  productArr.push(Math.round(Math.random()*20))
}
...
const productTypesArr = ['Couch', 'Coffee Table', 'End Table', 'TV Stand', 'Love Seat', 'Rug', 'Lamp', 'Shelf','Bed', 'Night Stand', 'Dresser', 'Desk', 'Chair', 'Wardrobe', 'Cabinets', 'Pots & Pans', 'Dishes', 'Appliances', 'Dining Table', 'Dining Chair']
  ...
  const products = await Promise.all(
    productArr.map(num =>{
     ...
     if(diningRoomTypes.includes(product)){
      ...
      return(
        Product.create({name: name , description: faker.lorem.paragraph(), quantity: Math.round(Math.random()*50), cost: faker.commerce.price(), imageUrl: productImgs[num][(Math.round( Math.random() * 6 ) ).toString() ] , roomId: room })
      )
    })
  );
